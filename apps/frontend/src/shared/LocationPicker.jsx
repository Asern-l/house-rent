  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-stone-500">在地图上点击选择位置（自动填入地址）</span>
        {status === 'loading' && (
          <span className="text-xs text-stone-400 animate-pulse">正在解析地址…</span>
        )}
        {status === 'done' && (
          <span className="text-xs font-medium" style={{ color: '#A47864' }}>✓ 地址已填入</span>
        )}
        {status === 'error' && (
      ...
      <div
        ref={containerRef}
        className="w-full overflow-hidden rounded-2xl border border-stone-300"
        style={{ height: 260 }}
      />
      <p className="text-[11px] text-stone-400">
        点击地图后会自动填入下方地址，你也可以手动修改。
      </p>
    </div>
  );
}
