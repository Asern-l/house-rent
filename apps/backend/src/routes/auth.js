...
router.post('/login', asyncHandler(async (req, res) => {
  try {
    const { walletAddress, signature, message, timestamp, nonce, role = 'tenant', nickname = '', phone = '' } = req.body || {};
    ...
    if (!nonceEntry) {
      return res.status(400).json({ error: 'nonce 无效或已过期，请刷新后重试' });
    }
    ...
    if (!Number.isFinite(ts) || Math.abs(Date.now() - ts) > SIGN_TIME_SKEW_MS) {
      return res.status(400).json({ error: '签名已过期，请刷新后重试' });
    }
    const expectedMessage = createLoginMessage(normalizedWallet, ts, nonce);
    if (String(message || '') !== expectedMessage) {
      return res.status(400).json({ error: '签名消息不匹配' });
    }
    let recovered = '';
    try {
      recovered = ethers.verifyMessage(message, signature);
    } catch {
      return res.status(400).json({ error: '签名验签失败' });
    }
    if (recovered.toLowerCase() !== normalizedWallet.toLowerCase()) {
      return res.status(400).json({ error: '签名钱包与提交地址不一致' });
    }
