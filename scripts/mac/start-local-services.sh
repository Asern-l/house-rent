...
echo "Starting Local services (three new Terminal windows will open)..."
echo "Note: start scripts/mac/start-persistent-local-node.sh first."
...
open_terminal "cd \"$FRONTEND_DIR\" && export VITE_DEFAULT_NETWORK=local && npm run dev"

echo "Started auth(3005), backend-local(3002), frontend(3001)."
